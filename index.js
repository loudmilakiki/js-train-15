...
function calculateCircleArea(radius) {
  ...
  const circlSq = Math.PI * Math.pow(radius, 2);
  return circlSq;
...
function calculateHypotenuse(a, b) {
  ...
  const gipotenuse = Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2));
  return gipotenuse;
...
function roundObjectValues(obj) {
  ...
  const newObj = enter.map(([key, value]) => {
...
function calculateVolumeCylinder(radius, height) {
  ...
  const volumeCilinder = Math.ceil(Math.PI * Math.pow(radius, 2) * height);
...
function getFractionalPart(num) {
  ...
  const remaind = Math.fround(num - newNum);
  return remaind;
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  const total = Math.fround(totalPrice);
  const max = Math.fround(maxPrice);
  return total > max;
